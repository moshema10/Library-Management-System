...
/**
 * The class TestIssueBook is the Junit5 test for testing Issue/Return Book operation.
 *
...
class TestReturnBook {
    ...
    @Test
    @Order(3)
    @DisplayName("Test read 'bookcopyDB.txt' after issuing books")
    ...
    void testReturnBook() {
        ...
        assertEquals(Book.StatusCode.ITEM_NOT_FOUND, lbr.returnBook("100", "1", "1234"),
                "No such book exists");
        assertEquals(Book.StatusCode.ITEM_NOT_FOUND, lbr.returnBook("002", "2", "1234"),
                "Book '002' has no edition #2");
        assertEquals(Book.StatusCode.ITEM_NOT_SIGNED_OUT, lbr.returnBook("002", "1", "7890"),
                "Book '002' not signed out by userid '7890'");
        assertEquals(Book.StatusCode.SUCCESS, lbr.returnBook("002", "1", "2345"),
                "Return second copy of Book '002'");
        assertEquals(Book.StatusCode.SUCCESS, lbr.returnBook("003", "1", "4567"),
                "Return first copy of Book '003'");
    ...
    @Test
    @Order(5)
    @DisplayName("Test read 'bookcopyDB.txt' again after returning books")
    void testReadBookCopyDBAgain() {
        LocalDate today = LocalDate.now();
        LocalDate due = today.plusDays(21);
        String str1 = today.toString();
        String str2 = due.toString();
        try (BufferedReader br = new BufferedReader(new FileReader("bookcopyDB.txt"))) {
            assertEquals("001        1  1  1 null       null       null      ", br.readLine());
            assertEquals("002        1  1  0 " + str1 + " " + str2 + " 1234      ", br.readLine());
            assertEquals("002        1  2  1 null       null       null      ", br.readLine());
